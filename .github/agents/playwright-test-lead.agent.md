...
| `playwright-test-planner` | Test architect | a target URL/feature | a plan in `test-plan.md` |
| `playwright-test-generator` | Implements the plan | a plan file | tests + page objects under `src/tests/` and `src/page-objects/` |
...
Load and apply [`AGENTS.md`](../../AGENTS.md) before coordinating anything. It is **non-negotiable
law** for every stage (locator strategy, Page Object Model, `ElementContainer<T>`, Controller
pattern, fixture-based dependency injection, Dependency Inversion & loose coupling, Clean Code &
SOLID, static quality gates, anti-pattern blocklist). Every handoff must preserve these rules. If a
rule conflicts with anything, `AGENTS.md` wins.
...
ticket intake ─▶ plan ─▶ generate ─▶ heal ─▶ verify ─▶ local review ─▶ git/PR ─▶ board move
   │            │        │           │       │          │              │          │
 story    test-plan.md src/tests/ src/tests/ npm run    agreed         rebase/    In Progress
 → tasks                                   check      fix-list       squash/PR  → Code Review
```
...
2. **Plan stage** — delegate to `playwright-test-planner`. Gate: a plan exists (default:
   ...
   - **Sizing:** if the plan implies a PR too large to review quickly (§14), have `jira-workflow`
...
3. **Generate stage** — hand the plan to `playwright-test-generator`. Gate: tests + page objects
   created under `src/tests/` and `src/page-objects/`, no anti-patterns, locators per `AGENTS.md`,
   DI via fixtures (`src/fixture/`).
...
5. **Verify** — run `npm run check` (typecheck + lint) and the test run. Green per the Definition of
   Done in `AGENTS.md`.
...
7. **Git/PR stage** — delegate to `git-workflow`: rebase onto `master`, resolve conflicts, Conventional
   Commits, squash, and open the PR (branch `test/<feature-kebab>` or `test/<ticket-id>-<feature-kebab>`).
   **Push and PR creation are gated on explicit user approval.** When the PR is raised,
   ...
   merges manually on GitHub after 2 approvals (§14).
...
- Tell the user exactly which agent to invoke next and with what prompt, e.g.
  `@playwright-test-generator implement the plan in test-plan.md`.
...
- **Enforce the gates, don't bypass them.** If `npm run check` is red, the healer (or generator)
...
- `npm run check` is green (TypeScript strict + ESLint, zero errors).
- Local review findings are discussed and resolved per the agreed fix-list.
- Branch is rebased onto `master`, squashed, Conventional Commits; PR raised (all gated on approval).
- Code honors Clean Code, SOLID, Dependency Inversion, Page Object Model, Controller pattern, and
  fixture DI per `AGENTS.md` — no anti-patterns from the blocklist.
